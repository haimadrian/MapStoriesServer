package org.bravetogether.mapstories.server.security;
...
public class JwtUtils {
   ...
   public User parseToken(String jwtToken) {
      ...
      try {
         ...
         logger.error("Error has occurred while parsing JWT token: " + e.toString(), e);
      }
...
//
//                  String keyStorePath = keyStoreFile;
//                  int indexOfColon = keyStorePath.indexOf(':');
//                  if (indexOfColon >= 0) {
//                     keyStorePath = keyStorePath.substring(indexOfColon + 1);
//                  }
//
//                  char[] pwdArray = keyStorePass.toCharArray();
//                  ks.load(getClass().getClassLoader().getResourceAsStream(keyStorePath), pwdArray);
//
//                  key = ks.getKey(keyStoreAlias, pwdArray);
                  logger.info("Key has been loaded and ready for use by JwtUtils");
               } catch (Exception e) {
                  logger.error("Error has occurred while loading key store: " + e.toString(), e);
               }
            }
         }
      }
